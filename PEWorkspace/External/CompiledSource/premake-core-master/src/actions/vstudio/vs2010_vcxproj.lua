...
--
	...
	function m.resourceCompile(cfg)
		--pe:akovalovs: do we need anything here for PSVita or PS4?
...
---------------------------------------------------------------------------
	...
	function m.entryPointSymbol(cfg)
		if cfg.entrypoint then
			...
			if (cfg.kind == premake.CONSOLEAPP or cfg.kind == premake.WINDOWEDAPP) and
				...
				--pe:akovalovs: do we need to do the same for PSVita or PS4?
			then
	...
	function m.platformToolset(cfg)
		...
		if cfg.toolset == 'snc' then --pe:akovalovs: for snc and clang, th versions are nil
		    m.element("PlatformToolset", nil, "SNC")
		    return
		end
		if cfg.toolset == 'clang' then
		    m.element("PlatformToolset", nil, "Clang")
		    return
		end
		if not version then
			local action = p.action.current()
