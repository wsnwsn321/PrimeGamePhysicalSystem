...
//#include "PrimeEngine/Utils/PhysicsReferenceList.h"
#include <vector>
std::vector<PE::Components::PhysicsComponent*> StaticObject;
	...
	if (!haveObject)
	{
		...
		else
		{
			if (pRealEvent->hasCustomOrientation)
			{
				...
				pSN->m_base.setV(pRealEvent->m_v);
				pSN->m_base.setN(pRealEvent->m_n);
				//adding PhysicsComponent
				PE::Handle hPC("PhysicsComponent", sizeof(PhysicsComponent));
				PhysicsComponent *pPC = new(hPC) PhysicsComponent(*m_pContext, m_arena, hPC);
				pPC->addDefaultComponents();
				pPC->objectName = pRealEvent->m_meshFilename;
				//only push plane base into the array
				if (strcmp("cobbleplane.x_pplaneshape1_mesh.mesha", pPC->objectName) == 0&& pSN->m_base.getPos().getY()>0) {
					pPC->m_pc_base = pSN->m_base;
					pPC->m_meshPoints.push_back(Vector3(-1250.0, -0.0, -1250.0));
					pPC->m_meshPoints.push_back(Vector3(1250.0, -0.0, -1250.0));
					pPC->m_meshPoints.push_back(Vector3(1250.0, 0.0, 1250.0));
					pPC->m_meshPoints.push_back(Vector3(-1250.0, 0.0, 1250.0));
					StaticObject.push_back(pPC);
				}
