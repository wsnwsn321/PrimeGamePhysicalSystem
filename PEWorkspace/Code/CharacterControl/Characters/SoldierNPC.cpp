...
//#include "../Code/PrimeEngine/Utils/PhysicsReferenceList.h"
...
SoldierNPC::SoldierNPC(PE::GameContext &context, PE::MemoryArena arena, PE::Handle hMyself, Event_CreateSoldierNPC *pEvt) : Component(context, arena, hMyself)
{
	...
	for (int iSkin = 0; iSkin < numskins; ++iSkin)
	{
		...
		pPC->m_vertexArray = pMeshInstance->m_vertexArray;
		for (int i = 0; i < pPC->m_vertexArray.size(); i=i+3)
		{
			Vector3 position = Vector3(pPC->m_vertexArray[i], pPC->m_vertexArray[i + 1], pPC->m_vertexArray[i + 2]);
			pPC->m_meshPoints.push_back(position);
		}
		m_meshPoints = pPC->m_meshPoints;
	...
	pSoldierMovementSM->meshName = pEvt->m_meshFilename;
	pSoldierMovementSM->start(pPC->m_meshPoints);
