		...
		
		{
			...
			if (pGameObjectManagerAddon)
			{
				...
				if (1)
				{
					...
					for (int i = 0; i < 1; ++i) {
						Vector3 pos2 = base*pSol->m_meshPoints[i];
						Vector3 target2 = Vector3(0, 0, 0);
						Vector3 color12(1.0f, 0.0f, 0);
						Vector3 color22(1.0f, 1.0f, 0);
						Vector3 linepts2[] = { pos2, color12, target2, color22 };
						//Matrix4x4 base = Matrix4x4();
						DebugRenderer::Instance()->createLineMesh(false, base, &linepts2[0].m_x, 2, 0);
					}
					sent = true;
				}
			}
